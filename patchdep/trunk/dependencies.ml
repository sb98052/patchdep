...
type depmap = ((patchname*patchname),(commit ref * commit ref) list) Hashtbl.t
type changemap = (filename,stamp list) Hashtbl.t

let change_map = Hashtbl.create 1024
let dep_map = Hashtbl.create 1024

let add_dependency mypatch oldpatch myblock oldblock =
  printf "Adding dependency %s->%s\n" mypatch oldpatch;flush Pervasives.stdout;
...
let normalize_dependencies patch newcommit cur_stamplist =
  ...
  let normalize arg_newcommit displacement cur_stamplist oldstamp =
    ...
    let (adddep,normalized_oldcommit) = 
      ...
      if (start1+count1 < !oldcommit.st) then
        begin
          oldcommit:={st = !oldcommit.st + displacement;num = !oldcommit.num};
          (false, oldcommit)
        end
       ...
       *)
        ...
        begin
          ...
          if (start1+count1 < !oldcommit.st + !oldcommit.num) then
            begin
              oldcommit:={st = start1;num = !oldcommit.st+ !oldcommit.num-start1};
              (true, oldcommit)
            end
          else
            begin
              oldcommit:={st = start1;num=count1};
              (true,oldcommit)
            end
        end
      (* we're somewhere in the middle of the old patch *)
        ...
        begin
          if (start1+count1 < !oldcommit.st + !oldcommit.num) then
            begin
              oldcommit:={num = !oldcommit.num + displacement;st = !oldcommit.st};
              (true, oldcommit)
            end
          else
            begin
              oldcommit:={num=start1+count1- !oldcommit.st;st = !oldcommit.st};
              (true, oldcommit)
            end
        end
      else
    ...
    in
      if (adddep) then
        add_dependency patch oldpatch newcommit normalized_oldcommit; 
  ...
  in
    match newcommit with
      ...
      | ChangeSpec((start1,count1),(_,count2)) -> 
          let c_newcommit =ref  {st=start1;num=count1}  in
...
let accept_commits patch fs =
  match fs with
    | FileSpec(str, commitlist) ->
        ...
        let calc_stamp arg_cur_stamplist newcommit =
          let (stamp0,arg_cur_stamplist0) = normalize_dependencies patch newcommit arg_cur_stamplist in
            stamp0::arg_cur_stamplist0
        in
        let new_stamplist = List.fold_left calc_stamp cur_stamplist commitlist in
